...
def get_block_subpixel(video, frame_number, dimensions, colour_component, kernel):
  ...
  expanded_dimensions[0] -= expanded_lines
  expanded_dimensions[1] -= expanded_lines
  expanded_dimensions[2] += kernel.shape[1] - 1
  expanded_dimensions[3] += kernel.shape[1] - 1
  ...
  if expanded_block.shape[1] < expanded_dimensions[2]: #right
    right_most_column = expanded_block[:, -1].reshape((-1, 1))
    expanded_block = np.column_stack((expanded_block, np.tile(right_most_column, (1, expanded_dimensions[2] - expanded_block.shape[1]))))
    print(expanded_block.shape)
  if int(dimensions[1]) < expanded_lines: #top
    ...
    print(expanded_block.shape)
  if expanded_block.shape[0] < expanded_dimensions[3]: #bottom
    bottom_most_row = expanded_block[-1, :].reshape((1, -1))
    expanded_block = np.row_stack((expanded_block, np.tile(bottom_most_row, (expanded_dimensions[3] - expanded_block.shape[0], 1))))
    print(expanded_block.shape)
