...
def FillMainReferenceLine(blockWidthReferenceLine, blockSize, modeId, filterTaps = 6):
  ...
  # Top part
  middlePart = blockWidthReferenceLine[0, 0:blockWidth*2+1]

  # Extend to the right
  extendedRight = [middlePart[-1]] * filterTaps
  ...
  if intraAnglePredMode >= 0:
    ...
    leftPart[-(maxLeftOffset-1):0] = middlePart[0]
  else:
    for deltaX in range(-blockHeight+1, 0):
      idx = blockHeight + 1 + deltaX
      pixelPosition = (-deltaX * invAngTable[intraAnglePredMode] + 8) // 16
